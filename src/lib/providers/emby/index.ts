...
export class EmbyProvider implements MediaProvider {
  ...
  async getGenres(auth?: AuthContext): Promise<MediaGenre[]> {
    ...
    const items = await getCachedGenres(auth.accessToken, auth.deviceId, auth.userId, auth.serverUrl);
  ...
  async getYears(auth?: AuthContext): Promise<MediaYear[]> {
    ...
    const items = await getCachedYears(auth.accessToken, auth.deviceId, auth.userId, auth.serverUrl);
  ...
  async getRatings(auth?: AuthContext): Promise<MediaRating[]> {
    ...
    try {
      ...
      return ratings.map((r: string) => ({ Name: r, Value: r }));
    } catch (error) {
  ...
  async getLibraries(auth?: AuthContext): Promise<MediaLibrary[]> {
    ...
    const items = await getCachedLibraries(auth.accessToken, auth.deviceId, auth.userId, auth.serverUrl);
    return items
